...
//필요
  ...
  //추가 시작
  callBalance: async function (walletInstance) {
    const userBalance = await cav.klay.getBalance(walletInstance.address)

    console.log(walletInstance.address);
    console.log(userBalance);
    console.log(typeof(walletInstance.address));
    return await cav.utils.fromPeb(userBalance, "KLAY");
  },
  //추가 끝
  ...
  changeUI: async function (walletInstance) {
    ...
    $("#login").hide(); 
    $('#logout').show();
    $('#collapsibleNavbar2').hide();
    // 추가 시작
    //$('#address').append('<br>' + '<p>' + '내 계정 주소: ' + walletInstance.address + '</p>'); 
    //$('#address').append('<br>' + '<p>' + '내 계정 주소: ' + await cav.klay.getBalance(walletInstance.address)+'</p>');
    //const userBalance = await cav.klay.getBalance(walletInstance.address)
    //console.log(walletInstance.address);
    //console.log(userBalance);
    //cav.utils.fromPeb(await this.callContractBalance(), "KLAY")

    //await this.callBalance()) + '</p>');
  ...
  receiveKlay: function() {
    ...
    agContract.methods.transfer(cav.utils.toPeb("2", "KLAY")).send({
      from: walletInstance.address,
      gas: '250000'
    }).then(function (receipt) {
      if (receipt.status) {
        spinner.stop();  
        alert("2 KLAY가 " + walletInstance.address + " 계정으로 지급되었습니다.");      
        $('#transaction').html("");
